...
`components/dispatch/call-transcript.tsx` now uses that real, cited quote — word-for-word matching the SF entry in `lib/case-studies.ts` and `docs/CASE_STUDIES.md`, all three tracing to the same primary source PDF. Two things keep this honest rather than just "real for the sake of real": (1) the quote is visually and structurally distinct from the framing lines around it (rendered as an attributed quotation, not attached to a fake "Caller"/"Dispatcher" label — the DVDRT report is systemic-review prose, not a call recording, and presenting it as spoken dialogue would misattribute it) and (2) the citation underneath links to `#case-study-sf`, jumping straight to the full, contextualized case study card further down the page (now has a stable anchor `id`, added to `case-study-card.tsx` for this). The badge/waveform/timer above the transcript stay abstract and generic — they represent the concept of "a 911 call," true of any of the ~1.65M incidents in this project, not a claim that this is a recording of *that* 2014 call.
