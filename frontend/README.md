...
**Milestone 11 update — the waveform now reacts to the transcript.** Previously `call-waveform.tsx` was purely decorative: a looping "someone is speaking" motif with no relationship to anything else on screen. `CallWaveform` now takes an `intensity: "idle" | "speaking" | "quote"` prop that scales each bar's amplitude and playback speed, and `CallTranscript` reports its most-recently-revealed line upward via an `onActiveLineChange` callback so `call-chapter.tsx` can derive that intensity from whatever the transcript is actually doing at that moment — calm before any text has appeared, normal for the framing lines, and most pronounced for the one line that's an actual verbatim excerpt from the real call record. This still adds no audio (real or fabricated) and no invented dialogue; it only makes an existing generic motif track the real content beats already being revealed, rather than animating obliviously regardless of what the reader is currently seeing.
